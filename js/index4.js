...
function funcInput(){
    ...
    if (serNum.length !== 19) {
        isValid = false;
        message.innerHTML = "Regle 1 : Mauvaise longueur !";
    } else if (serNum[4] !== '-' || serNum[9] !== '-' || serNum[14] !== '-') {
        isValid = false;
        message.innerHTML = "Regle 1 : Mauvais format (tiret) !";
    } else if (isNaN(serNum.split('-').join(''))) {
        isValid = false;
        message.innerHTML = "Regle 1 : Mauvais format (chiffres) !";
    } else if (parseInt(serNum.split('-').join('')) !== parseFloat(serNum.split('-').join(''))) {
        isValid = false;
        message.innerHTML = "Regle 1 : Mauvais format (y'a un point) !";
    }
